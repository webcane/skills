---
name: hypothes-to-concept
description: >
  Analyze, clarify, reformulate, or solve a problem using a structured reasoning method.
  Use when you need to stress-test a proposal, find hidden assumptions, simplify a concept,
  resolve a contradiction, validate a decision, or converge a raw idea into a well-formed
  concept. Two modes: auto-recommend the best-fit method from the problem description, or
  explicit method selection (user choice or input parameter). Usable as a sub-agent by
  other skills — pass a structured analyze: block and receive a structured result.
metadata:
  author: webcane
  version: 1.0.0
...
# Hypothes-to-Concept — Structured Reasoning Skill
...
The user describes the problem directly, or invokes `/hypothes-to-concept <problem>`.
