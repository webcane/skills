...
figure_scale, split, and seamless) only appears if `layers.figure.<group>` is a type
value (not `"false"`; transformation decks); otherwise they're part of step 6 and simply not
appended.
...
- **`illustration`** — exactly five changes to assembly, all others unchanged:
  ...
  4. **Append a fixed block to `[NEGATIVE_LIST]`**, after the usual exclusions:
     ...
     symbols`
  5. **Drop title and seamless instructions.** Under `structure: illustration`, no
