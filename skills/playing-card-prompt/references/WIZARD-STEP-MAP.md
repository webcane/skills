...
| **6** | Card decoration & theme | #14 Decoration/ornamentation, #15 Deck-wide theme/symbolism, #5 Composition/rhythm *(partial)* | Decor.pip, Ornaments.pip, Frame.pip (toggle); Highlights.\* (+extra); `ornaments_extra`/`highlights_extra`/`frame_extra` (incl. theme-derived) | feeds `[STYLE_BLOCK]` (extra phrases), `[FRAME_LINE]` (preset text + frame_extra) | `assets/frame/<frame>.md`, `assets/frame/_adding-a-frame.md` (if custom), `references/REFERENCE.md` ("Theme-derived ornaments/highlights/frame") |
| **7** | Mood / atmosphere | #4 Mood/atmosphere | Mood.\* (per-group toggle); `mood` (preset or custom text) | feeds `[STYLE_BLOCK]` (`[MOOD_LINE]`) | `assets/mood/<name>.md`, `assets/mood/_adding-a-mood.md` (if custom) |
