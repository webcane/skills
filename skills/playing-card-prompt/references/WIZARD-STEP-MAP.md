...
| **5** | Visual style/pattern | #1 Medium/technique, #2 Color palette, #3 Era/cultural context, #6 Line character, #7 Degree of stylization *(partial)*, #11 Typage/facial expression | Background, Decor, Ornaments, Highlights, Technique, Figure detail, Face Style, Finish — supplies the TEXT for all of them | feeds `[STYLE_BLOCK]` (layer text, incl. Technique when `layers.technique.<group>` is on, and Figure detail/Face Style when `layers.figure.<group>` is on) | `assets/pattern/<style>.md`, `assets/pattern/_adding-a-pattern.md` (if custom) |
...
- **`layers.technique.<group>`** — on for every group by default, contributing the chosen pattern's "Technique" section to `[STYLE_BLOCK]`; toggling it off per group, or setting a per-group addition, is config-only (like `layers.background`/`layers.decor` additions), not asked in the wizard.
