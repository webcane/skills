...
- **No valid config found** (`config.json` is absent — the normal state for a freshly
  installed skill, since `config.json` is not shipped or git-tracked — or was just reset
  via `manage_config.py reset`; treat both identically, no separate messaging) → do NOT
...
- **Angled mirrored** — two mirror-image halves split along a 15-degree
  diagonal axis corner-to-corner, meeting along that diagonal seam. Sets
