...
- **Active split modes rendered a duplicate horizontal line, an internally-contradictory angled-mirrored angle, and a run-on split/seamless collision (SPLT-03, Phase 01 UAT Test 6 gap, debug session `split-seamless-angle-collision`)** — three defects, all confirmed by re-test after the prior SPLT-03 fix: (A) `references/REFERENCE.md`'s COURT template hardcoded an unconditional `thin black horizontal dividing line ... rotated 180 degrees ...` clause outside `[STYLE_BLOCK]`, never gated on `layers.split.court`, so an active split mode rendered BOTH that horizontal-180 clause AND the diagonal split phrase at once. Replaced the two hardcoded lines with a `[COURT_SPLIT_DEFAULT_LINE]` placeholder that expands to that text only when `layers.split.court` is `"false"`/`"none"`, and to empty otherwise. (B) `assets/split/angled-mirrored.md`'s split line paired a bare unreferenced `15-degree` angle with `corner-to-corner`, which is geometrically ~35-55 degrees on the card's actual aspect ratio — the two qualifiers contradicted each other, so the model discarded `15-degree` and rendered the spatially-grounded ~45-degree corner-to-corner reading instead. Dropped the numeric angle in favor of consistent qualitative diagonal framing (`mirrored halves split along a steep corner-to-corner diagonal axis, meeting along that diagonal seam,`), synced into SKILL.md Step 8b's matching option text. (C) `REFERENCE.md`'s `[STYLE_BLOCK]` assembly (steps 6d/6e) appended split and seamless phrases as an undifferentiated run-on landing immediately before `[CHARACTER_FEATURES]`; step 6e now introduces the seamless phrase with an explicit transition lead-in (e.g. `and, as a deck-level edge treatment, ...`) whenever both split and seamless are active, so the two read as distinct instructions.
...
### Changed
- **Phase 5 structural-cleanup summary (UNIFY-01..04, TITLE-01..03, BACK-EPH-01..02, WIZ-01..03)** — every `layers.<layer>.<group>` cell is unified onto one `false|true|alias|custom` contract; a stored `"true"` now means "layer active, resolved fresh per-card" rather than being eagerly resolved to a fixed default at config-read time, and is stored verbatim (`figure` is no longer a strict enum and now accepts custom free-text figure descriptions). `mood` is documented as bool-only in the normal wizard flow, with raw custom text available only as a `manage_config.py set` escape hatch. Title and Card-back design criteria moved from deck-wide persistent config fields to per-card ephemeral wizard steps (asked fresh every card, never saved); title gains group suggested-default placement aliases sourced from new `assets/title/*.md` presets (Joker -> `below-figure`, Court -> `side-running`). Wizard structure simplified: `SKILL.md` Steps 3, the figure-check, and Step 6 now point to `references/REFERENCE.md`'s Defaults table instead of restating per-(layer,group) defaults inline; Steps 8a/8b/8c/8e/8f and Step T use a consistent enumerated `_Skip this step if ANY of:_` format; and the `structure` (full vs. illustration) question moved from a config-only field to an early wizard step (new Step 1b, right after Deck type), with its downstream consequences explained once there instead of re-derived at Step 8f/Step T/assembly — **this question-order change is the one user-facing flow change** in the phase.
- **`SKILL.md` per-card-resolution wizard updates (UNIFY-01/02, TITLE-01/02/03, BACK-EPH-01)** — Step T rewritten as a per-card ephemeral title step gated only by `structure`, with group suggested-default placement aliases (Joker -> `below-figure`, Court -> `side-running`, pip/ace/back/special -> no title suggested) and the dead `title.enabled` gate removed; the startup persistent-settings list drops `title.enabled` and the five `back_*` fields, with the per-card "asked fresh, never saved" sentence now naming title text/placement and the back design criteria; Step 3's Back routing note clarifies B1-B5 are per-card ephemeral, B6 sets the persistent `layers.frame.back` cell, and B7 is per-card; the figure-check after Step 7 and Steps 8b (split)/8c (figure type)/8f (seamless) are rewritten for the unified `false|true|alias|custom` contract — a `"true"` cell means the layer is active for this group with the concrete alias/custom text resolved fresh per-card, not eagerly defaulted at config-read time; Step 8c also drops the `strict=True`-only-accepts-the-keyword-`custom` restriction now that the figure layer accepts a literal custom free-text description directly. No version bump in this change — deferred to the next `SKILL.md`-touching plan in this phase, which performs the wizard-structure cleanup pass and bumps once for the whole change set.

### Removed
- **`title.enabled` persistent field and the five `back_*` persistent fields** (`back_purpose`, `back_design`, `back_pattern`, `back_palette`, `back_symmetry`) — no longer valid `manage_config.py` keys; a load-path migration (`_migrate_drop_phase5_persistent()`) silently drops these stale keys from any existing `config.json` so older profiles still pass `validate` after upgrading.

### Added
- **`assets/title/` placement presets** — added `below-figure`, `side-running`, `top-banner`, and `corner-plate` title placement presets plus an `_adding-a-title.md` authoring guide, discoverable via `manage_config.py`'s `_discover('title')` helper. These back an upcoming per-card ephemeral title wizard step (D-01/D-02): the file stems are offered as per-card suggested defaults by group (Joker -> `below-figure`, Court -> `side-running`), not persisted to `config.json` — title has no `layers.title.<group>` config cell. No wizard or schema changes in this step; asset files only.

### Changed
- **`references/WIZARD-BACK.md`, `references/WIZARD-STEP-MAP.md`, and `references/STYLE-COMPONENTS.md` rewritten for the ephemeral title/back model (BACK-EPH-01/02, TITLE-02/03)** — `WIZARD-BACK.md` Steps B1-B5 (purpose, design category, pattern, palette, symmetry) drop their "skipped if already set in config" guard and `manage_config.py set` persistence, documented instead as asked fresh every Back card (per-card, never saved), while keeping the back_design-to-back_pattern category-gated dependency and the D-21 geometric fallback intact; B6 (frame) and B7 (exclusions) are unchanged. `WIZARD-STEP-MAP.md`'s Title row (T) drops the dead `title.enabled` deck-wide-gate framing and instead names `assets/title/<alias>.md` as its asset lookup with the Joker `below-figure` / Court `side-running` suggested defaults; Back rows B1-B5 are relabeled per-card (B6/B7 unchanged); the structure footnote drops the `title.enabled` reference while keeping the seamless gating clause; the Back rank-group footnote is updated to match. `STYLE-COMPONENTS.md`'s #5 row gains a clause covering the per-card title placement mechanism, folded into the existing row (no new component number). Doc-only — no code change; brings these three wizard-flow docs in line with the schema 05-02 already shipped.
- **`references/REFERENCE.md` and `references/CONFIG.md` rewritten to document the unified layer contract (UNIFY-01/02/03/04, TITLE-01/03, WIZ-01)** — both reference docs now match the unified `manage_config.py` schema exactly: the figure layer's `false|true|alias|custom` contract is stated explicitly (no more "strict, no addition slot" framing), the split `"true"`→`"none"` and seamless `"true"`→default-alias eager-resolution prose is replaced with per-card-resolution wording, `layers.mood.<group>` is documented as bool-only in the normal wizard flow with raw text framed as a `manage_config.py set` escape hatch (not asked in the wizard), the `title.enabled` field and the five `back_*` persistent fields are removed from both docs (schema example, field-reference table, persistent-settings list), the "Back design criteria fields" section is replaced with a per-card ephemeral note pointing at `WIZARD-BACK.md` and the kept `allowed_back_patterns()` helper, the "Title text" section is rewritten for the ephemeral per-card model gated only by `structure`, and REFERENCE.md's "Defaults" table is declared the single canonical `(layer, group)` default source that CONFIG.md now points to instead of restating. Doc-only — no code change; brings the docs in line with the schema 05-02 already shipped.
- **Unified the `layers.<layer>.<group>` value contract across every layer (UNIFY-01/02/03)** — `manage_config.py`'s `options_for()` no longer special-cases `figure` (strict enum) or `split`/`seamless` (eager `"true"`-resolution comments); every layer, figure included, now returns `strict=False` under one `false`/`true`/`<alias>`/`<custom_text>` contract. A literal `"true"` written via `cmd_set` is now persisted verbatim for every layer — the write-path block that immediately rewrote `figure`'s `"true"` to its `LAYER_DEFAULTS` alias and `split`'s `"true"` to `"none"` is removed. Custom free-text figure descriptions (e.g. `"a weathered stone gargoyle"`) are now valid, not just the `character`/`building`/`animal`/`custom` enum. `"true"` no longer resolves immediately to a fixed default the moment config is read/written — it means "layer active for this group," with the concrete alias/custom text deferred to per-card resolution (D-06/D-07). `_migrate_figure_true_to_character()`/`_migrate_seamless_true_to_alias()` are kept in the file unchanged but are no longer called from `load_raw()`'s automatic migration chain, since eagerly rewriting a stored `"true"` on every load now contradicts the new contract (a literal `"true"` is intentionally preserved, not a footgun to fix).
- **`title.enabled` and the five `back_*` fields removed from the persistent schema (TITLE-01/BACK-EPH-01)** — `title`, `back_purpose`, `back_design`, `back_pattern`, `back_palette`, and `back_symmetry` are no longer valid `manage_config.py` keys (removed from `DEFAULTS`, `PERSISTENT_KEYS`, and `options_for`'s fixed map); `manage_config.py set title.enabled ...` / `set back_purpose ...` now fail with "unknown key". Title and card-back design move fully to per-card ephemeral wizard steps (D-01/D-04) — nothing about them is read from or written to `config.json` anymore. `allowed_back_patterns()` and the `BACK_*` enum constants are kept as standalone helpers (no longer wired through `options_for`) so a future per-card wizard step can still list category-gated pattern aliases directly.
- **Added `_migrate_drop_phase5_persistent()`** — a new unconditional `load_raw()` migration that silently drops a stale `title` key and any of the five `back_*` keys from an existing profile, so a config.json written by pre-Phase-5 code (or hand-edited) still passes `validate` after upgrading. Does not touch `layers.*` cells.
...
- **Fresh installs skipped the first-run Config wizard because the shipped `config.json` already had a fully-populated `default` profile** — per `SKILL.md` Generate-mode logic, a present `config.json` short-circuits straight to per-card steps ("Config exists"), so the intended "no valid config found → run the full Config wizard" path only ever fired after an explicit `--reset`. Fixed by no longer git-tracking or packaging `config.json`: removed it from git (`git rm`), added `skills/playing-card-prompt/.gitignore` (`config.json`, `config.json.bak*`) so a developer's locally-generated config can't be re-committed, and added `--exclude='config.json'` to `scripts/package-skill.sh`'s `tar` invocation so a local file on disk isn't re-shipped in the `.skill` archive either. `manage_config.py` already handled an absent file correctly (`load_raw()` falls back to in-code `DEFAULTS`/`LAYER_DEFAULTS` as `BUILTIN_CONFIG`, and `cmd_show`/`cmd_validate` report "not created — using built-in defaults"), so no script behavior changed — only the shipped file state. Clarified `SKILL.md`'s Generate-mode "No valid config found" bullet to note this is the expected first-run state for a fresh install, not only the post-`--reset` case.
