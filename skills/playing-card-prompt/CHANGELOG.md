...
- **`[BACK_DESIGN]` assembly** — now assembled from three asset lines: purpose line + pattern line + palette line. Symmetry is excluded from `[BACK_DESIGN]` (goes via STYLE_BLOCK step 10); frame excluded (goes via `[FRAME_LINE]`). `[BACK_MODIFICATIONS]` placeholder removed from BACK template.
...
- **WIZARD-STEP-MAP.md** updated: B1–B3 rows replaced with B1–B7 rows.
- **CONFIG.md** updated: five new `back_*` persistent fields documented.
